...
def value_evaluation(size, obstacles, policy):
...
def generate_value_matrix():
    ...
    value_matrix = value_evaluation(size, obstacles, policy)
    return jsonify({'value_matrix': value_matrix})

def value_iteration(size, obstacles):
    rewards = np.full((size, size), -0.001)
    value_matrix = np.zeros((size, size))
        ...
        for i in range(size):
            for j in range(size):
                ...
                for idx, (dx, dy) in enumerate(actions):
                    ni, nj = i + dx, j + dy
                    if 0 <= ni < size and 0 <= nj < size:
                        value = rewards[ni, nj] + discount_factor * value_matrix[ni, nj]
                    else:
                        value = rewards[i, j] + discount_factor * value_matrix[i, j]
                    
                    if value > best_value:
                        best_value = value
                        best_action = action_symbols[idx]
                
                new_value_matrix[i, j] = best_value
                policy[i][j] = best_action
                delta = max(delta, abs(best_value - value_matrix[i, j]))
        
        value_matrix = new_value_matrix
        if delta < threshold:
            break
    
    return value_matrix.tolist(), policy

@app.route('/run_value_iteration', methods=['POST'])
def run_value_iteration():
    size = int(request.json['size'])
    obstacles = set(tuple(ob) for ob in request.json.get('obstacles', []))
    value_matrix, policy = value_iteration(size, obstacles)
    return jsonify({
